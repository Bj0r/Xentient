...
- Class has `pendingTurns: PendingTurn[]` and `debounceTimer` fields (debouncing implemented)
- `extractAfterTurn` is NOT async (fire-and-forget queueing, not blocking)
- `extractAfterTurn` contains word-count guard: `if (wordCount < this.minWordCount)`
- `flush()` method exists for graceful shutdown
- `cd harness && npx tsc --noEmit` exits 0
</acceptance_criteria>
